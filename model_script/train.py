...
def cnn_train(datasets_dir, save_dir, epochs=20, lr=0.001, train_size_rate=0.8, batch_size=32, image_size=default_image_size, model_save_name="nahida_cnn"):
    # 1. 加载你的数据集
    ...
    for epoch in range(epochs):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), os.path.join(save_dir, model_save_name + "_best.pth"))
            print(f"  → 保存最优模型，准确率: {best_acc:.4f}")

    # 6. 保存模型
    torch.save(model.state_dict(), os.path.join(save_dir, model_save_name + ".pth"))
    #               仅保存权重
    print("模型已保存")
