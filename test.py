...
save_dir = 'download'
sub_dir = os.path.join(save_dir)  # download/nahida os会自动处理是 / 还是 \
os.makedirs(save_dir, exist_ok=True)
os.makedirs(sub_dir, exist_ok=True)


def link_to_image(image_name, pid_link):
    headers = {
...
def link_find(phpsessid, pid):
    ...
    # json_data = respo    nse.json()  # json()来自requests可以自动将json解析成字典
...
def image_download(phpsessid, pid):
    pid_link = pixiv_imagelink.link_find(phpsessid, pid)

    image_name = pid_link.split(sep='/')[-1]  # 112876385_p0.xxx

    for i in range(0, 10):
        temp_name = image_name.replace(image_name.split('_')[1], image_name.split('_')[1].replace('p0', 'p' + str(i)))
        temp_link = pid_link.replace(image_name, temp_name)
        if not link_to_image(temp_name, temp_link):
            break
